...
def layer_info(
    ...
    # Generate the model’s next predicted token and its token ID
    next_token, next_token_id = generate_next_token(
        model, 
        tokenizer, 
        in_seq, 
        device
    ...
    # Collect input/output activations for the specified layer
    layer_io, input_ids = collect_layer_input_output(
        model, 
        tokenizer, 
        layer_idx, 
        in_seq, 
    ...
    return reshape_W_E, U, S, Vh, layer_io, next_token, next_token_id, c_fc, c_proj, ln_2, act
